...
#                                          std=[x/255.0 for x in [63.0, 62.1, 66.7]])
    ...
    if out_dataset == 'SVHN':
        ...
        testsetout = torchvision.datasets.ImageFolder(root= os.path.join(root, 'places365', 'test_subset'),
            transform=transforms.Compose([transforms.Resize(img_size), 
            transforms.CenterCrop(img_size), transforms.ToTensor(),normalize]))
    else:
...
def set_ood_loader_ImageNet(args, out_dataset):
    ...
    if out_dataset == 'iNaturalist':
        ...
        testsetout = torchvision.datasets.ImageFolder(root= os.path.join(root, 'places365', 'test_subset'),transform=preprocess)  
    ...
    if len(testsetout) > 5000: 
        testsetout = torch.utils.data.Subset(testsetout, np.random.choice(len(testsetout), 5000, replace=False))
    testloaderOut = torch.utils.data.DataLoader(testsetout, batch_size=args.batch_size,
